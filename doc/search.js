...
function getURL(item, category) {
  ...
  if (category === "modules") {
    ...
    url +=
      checkUnnamed(item.p, "/").replace(/\./g, "/") + item.c + ".html" + "#";
    if (item.u) {
      url += item.u;
    } else {
      url += item.l;
...
function createMatcher(term, camelCase) {
  ...
  term
    .trim()
    .split(/\s+/)
    .forEach(function (w, index, array) {
      ...
      for (var i = 0; i < tokens.length; i++) {
        ...
        pattern +=
          "(" +
          $.ui.autocomplete.escapeRegex(s).replace(/[,?]/g, "$&\\s*?") +
          ")";
...
function findMatch(matcher, input, startOfName, endOfName) {
  ...
  while (
    !match ||
    match.index + match[0].length < startOfName ||
    endOfName < match.index
  ) {
    if (from === 0) {
      return NO_MATCH;
    }
...
function doSearch(request, response) {
  ...
  function searchIndex(indexArray, category) {
    ...
    $.each(indexArray, function (i, item) {
      ...
      var endOfName =
        category === "members" && input.indexOf("(", startOfName) > -1
          ? input.indexOf("(", startOfName)
          : input.length;
...
$.widget("custom.catcomplete", $.ui.autocomplete, {
  ...
  _renderMenu: function (ul, items) {
    ...
    ul.append(
      "<li class='ui-static-link'><a href='" +
        pathtoroot +
        "search.html?q=" +
        encodeURI(widget.term) +
        "'>Go to search page</a></li>"
    );
  },
...
$(function () {
  ...
  $("section[id] > :header, :header[id], :header:has(a[id])").hover(
    function () {
      $(this).append(
        $(
          "<button class='copy copy-header' onclick='copyUrl(this)'> " +
            "<img src='" +
            pathtoroot +
            "copy.svg' alt='" +
            messages.copyUrl +
