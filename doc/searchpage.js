  ...
  expand.click(function (e) {
    ...
    if (this.parentElement.hasAttribute("open")) {
      ...
      searchInfo
        ...
        .height(searchInfo.prop("scrollHeight"));
    }
  });
});
$(window).on("load", function () {
  ...
  function renderResults(result) {
    ...
    if (result.length > 20) {
      if (searchTerm[searchTerm.length - 1] === ".") {
        if (activeTab === "types" && r["members"].length > r["types"].length) {
          ...
          activeTab === "packages" &&
          r["types"].length > r["packages"].length
        ) {
          activeTab = "types";
        }
      }
    ...
    visibleTabs = [];
    var tabContainer = $("<div class='table-tabs'></div>").appendTo(
      resultContainer
    );
    for (var key in r) {
      ...
      if (r[key].length) {
        ...
        if (result.length > 20 && categoryCount > 1) {
          ...
          tabContainer = $("<div class='table-tabs'></div>").appendTo(
            resultContainer
          );
        }
      }
  ...
  function renderTable(category, items) {
    var table = $("<div class='summary-table'>").addClass(
      category === "modules"
    ...
    if (category !== "modules") {
      $("<div class='table-header col-plain'>" + col2 + "</div>").appendTo(
        table
      );
    }
  ...
  function renderItem(item, table, rowColor) {
    ...
    var link = $("<a/>")
      ...
      .html(label);
    var container = getHighlightedText(
      item.input,
      item.boundaries,
      0,
