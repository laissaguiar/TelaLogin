...
function makeComparable(s) {
  return s.toLowerCase().replace(/(\d+)/g, function (n, m) {
...
function sortTable(header, columnIndex, columns) {
  ...
  container.querySelectorAll("div.table-header").forEach(function (header) {
...
function toggleGlobal(checkbox, selected, columns) {
  ...
  document.querySelectorAll("div.table-tabs").forEach(function (t) {
    ...
    var visible = 0;
    document.querySelectorAll("div." + id).forEach(function (elem) {
...
function show(tableId, selected, columns) {
  if (tableId !== selected) {
    document
      .querySelectorAll("div." + tableId + ":not(." + selected + ")")
      .forEach(function (elem) {
        elem.style.display = "none";
      });
  }
  document.querySelectorAll("div." + selected).forEach(function (elem, index) {
    elem.style.display = "";
...
function updateTabs(tableId, selected) {
  document
    .querySelector("div#" + tableId + " .summary-table")
    .setAttribute("aria-labelledby", selected);
  document
    .querySelectorAll('button[id^="' + tableId + '"]')
...
function copyUrl(button) {
  ...
  if (header.hasAttribute("id")) {
    ...
    header.firstElementChild.tagName === "A" &&
